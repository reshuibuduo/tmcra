...
def create_app(settings: ServiceSettings) -> FastAPI:
    ...
    async def commercial_contract_error(
        ...
        if exc.code == "scope_deleted":
            status_code = 410
        elif exc.code in {"scope_deleting", "scope_content_deleting"}:
            status_code = 409
    ...
    async def recall(
        ...
        try:
            ...
            route = select_evidence_route(body.evidence_mode, evidence)
            compiled = None
    ...
    def submit_feedback(
        ...
        scope_name = _scope_name(scope_name)
        require_active_scope(context.tenant_id, scope_name)
        return components.commercial.add_feedback(
            context.tenant_id,
            scope_name,
