    ...
    def _run_with_writer_env(
        ...
        *,
        ...
        with log_path.open("x", encoding="utf-8") as log:
            ...
            log.flush()
            subprocess.run(
                ["bash", "-c", shell, "tmcra-service", str(self.settings.writer_env), *command],
                check=True,
                stdout=log,
...
class V4OnlineEngine:
    ...
    def _load_base_index_cached(
        ...
        return self._remember_index_bundle(
            key,
            self.runtime.load_online_index(index_path, database_path, scope_id),
        )
    ...
    def warmup(self, snapshots: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
        ...
        with self._lock, torch.inference_mode(), self._stream_context():
            ...
            if tuple(dense.shape) != (1024,) or not bool(torch.isfinite(dense).all()):
                raise RuntimeError("BGE-M3 startup probe returned an invalid vector")
