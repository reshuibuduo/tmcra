...
The supported production path provides **100% source-traceable recall
evidence**: every returned evidence block binds to its immutable Source ID,
actor, source application, and time coordinate. This guarantee covers the
evidence packet and its audit chain. A free-form answer generated by an outer
Agent remains the responsibility of that Agent and its application.
...
The default Writer, Reviewer, recall planner, and slow-graph route uses the
self-hosted `Qwen3.6-35B-A3B` model. DeepSeek Flash/Pro remains an optional
provider route. The operator controls model licensing, capacity, price, and
availability for every selected route.
